...
{
    [BurstCompile]
    public struct HighestScoreComponentGhostComponentSerializer
    {
        static GhostComponentSerializer.State GetState()
        {
            ...
            if (!s_StateInitialized)
            {
                s_State = new GhostComponentSerializer.State
                {
                    ...
                    ComponentType = ComponentType.ReadWrite<HighestScoreComponent>(),
                    ComponentSize = UnsafeUtility.SizeOf<HighestScoreComponent>(),
        ...
        private static void CopyToSnapshot(IntPtr stateData, IntPtr snapshotData, int snapshotOffset, int snapshotStride, IntPtr componentData, int componentStride, int count)
        {
            for (int i = 0; i < count; ++i)
            {
                ...
                ref var component = ref GhostComponentSerializer.TypeCast<HighestScoreComponent>(componentData, componentStride*i);
                ref var serializerState = ref GhostComponentSerializer.TypeCast<GhostSerializerState>(stateData, 0);
                snapshot.playerName = component.playerName;
                snapshot.highestScore = (int) component.highestScore;
            }
        }
        [BurstCompile]
        [MonoPInvokeCallback(typeof(GhostComponentSerializer.CopyToFromSnapshotDelegate))]
        private static void CopyFromSnapshot(IntPtr stateData, IntPtr snapshotData, int snapshotOffset, int snapshotStride, IntPtr componentData, int componentStride, int count)
        {
            for (int i = 0; i < count; ++i)
            {
                ...
                component.highestScore = (int) snapshotBefore.highestScore;
        ...
        {
            ref var component = ref GhostComponentSerializer.TypeCast<HighestScoreComponent>(componentData, 0);
            ref var backup = ref GhostComponentSerializer.TypeCast<HighestScoreComponent>(backupData, 0);
            component.playerName = backup.playerName;
            component.highestScore = backup.highestScore;
        }
        ...
        private static void PredictDelta(IntPtr snapshotData, IntPtr baseline1Data, IntPtr baseline2Data, ref GhostDeltaPredictor predictor)
        {
            ...
            snapshot.highestScore = predictor.PredictInt(snapshot.highestScore, baseline1.highestScore, baseline2.highestScore);
        }
        [BurstCompile]
        ...
        private static void CalculateChangeMask(IntPtr snapshotData, IntPtr baselineData, IntPtr bits, int startOffset)
        {
            ...
            uint changeMask;
            changeMask = snapshot.playerName.Equals(baseline.playerName) ? 0 : 1u;
            changeMask |= (snapshot.highestScore != baseline.highestScore) ? (1u<<1) : 0;
        ...
        private static void Serialize(IntPtr snapshotData, IntPtr baselineData, ref DataStreamWriter writer, ref NetworkCompressionModel compressionModel, IntPtr changeMaskData, int startOffset)
        {
            ...
            if ((changeMask & (1 << 1)) != 0)
                writer.WritePackedIntDelta(snapshot.highestScore, baseline.highestScore, compressionModel);
        }
        [BurstCompile]
        ...
        private static void Deserialize(IntPtr snapshotData, IntPtr baselineData, ref DataStreamReader reader, ref NetworkCompressionModel compressionModel, IntPtr changeMaskData, int startOffset)
        {
            ...
            if ((changeMask & (1 << 1)) != 0)
                snapshot.highestScore = reader.ReadPackedIntDelta(baseline.highestScore, compressionModel);
            else
                snapshot.highestScore = baseline.highestScore;
        }
        #if UNITY_EDITOR || DEVELOPMENT_BUILD
        [BurstCompile]
        ...
        private static void ReportPredictionErrors(IntPtr componentData, IntPtr backupData, ref UnsafeList<float> errors)
        {
            ...
            ref var backup = ref GhostComponentSerializer.TypeCast<HighestScoreComponent>(backupData, 0);
            int errorIndex = 0;
            errors[errorIndex] = math.max(errors[errorIndex], math.abs(component.highestScore - backup.highestScore));
            ++errorIndex;
        }
        private static int GetPredictionErrorNames(ref FixedString512 names)
        {
            ...
            names.Append(new FixedString64("highestScore"));
            ++nameCount;
            return nameCount;
        }
        #endif
