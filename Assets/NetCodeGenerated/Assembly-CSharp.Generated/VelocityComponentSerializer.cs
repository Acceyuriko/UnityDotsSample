...
{
    [BurstCompile]
    public struct VelocityComponentGhostComponentSerializer
    {
        static GhostComponentSerializer.State GetState()
        {
            ...
            if (!s_StateInitialized)
            {
                s_State = new GhostComponentSerializer.State
                {
                    ...
                    ComponentType = ComponentType.ReadWrite<VelocityComponent>(),
                    ComponentSize = UnsafeUtility.SizeOf<VelocityComponent>(),
        ...
        private static void CopyToSnapshot(IntPtr stateData, IntPtr snapshotData, int snapshotOffset, int snapshotStride, IntPtr componentData, int componentStride, int count)
        {
            for (int i = 0; i < count; ++i)
            {
                ...
                ref var component = ref GhostComponentSerializer.TypeCast<VelocityComponent>(componentData, componentStride*i);
                ref var serializerState = ref GhostComponentSerializer.TypeCast<GhostSerializerState>(stateData, 0);
                snapshot.Linear_x = component.Linear.x;
                snapshot.Linear_y = component.Linear.y;
                snapshot.Linear_z = component.Linear.z;
            }
        }
        [BurstCompile]
        [MonoPInvokeCallback(typeof(GhostComponentSerializer.CopyToFromSnapshotDelegate))]
        private static void CopyFromSnapshot(IntPtr stateData, IntPtr snapshotData, int snapshotOffset, int snapshotStride, IntPtr componentData, int componentStride, int count)
        {
            for (int i = 0; i < count; ++i)
            {
                ...
                ref var component = ref GhostComponentSerializer.TypeCast<VelocityComponent>(componentData, componentStride*i);
                component.Linear = new float3(snapshotBefore.Linear_x, snapshotBefore.Linear_y, snapshotBefore.Linear_z);
        ...
        {
            ref var component = ref GhostComponentSerializer.TypeCast<VelocityComponent>(componentData, 0);
            ref var backup = ref GhostComponentSerializer.TypeCast<VelocityComponent>(backupData, 0);
        ...
        private static void Serialize(IntPtr snapshotData, IntPtr baselineData, ref DataStreamWriter writer, ref NetworkCompressionModel compressionModel, IntPtr changeMaskData, int startOffset)
        {
            ...
            if ((changeMask & (1 << 0)) != 0)
                writer.WritePackedFloatDelta(snapshot.Linear_x, baseline.Linear_x, compressionModel);
            if ((changeMask & (1 << 0)) != 0)
                writer.WritePackedFloatDelta(snapshot.Linear_y, baseline.Linear_y, compressionModel);
            if ((changeMask & (1 << 0)) != 0)
                writer.WritePackedFloatDelta(snapshot.Linear_z, baseline.Linear_z, compressionModel);
        }
        [BurstCompile]
        ...
        private static void Deserialize(IntPtr snapshotData, IntPtr baselineData, ref DataStreamReader reader, ref NetworkCompressionModel compressionModel, IntPtr changeMaskData, int startOffset)
        {
            ...
            uint changeMask = GhostComponentSerializer.CopyFromChangeMask(changeMaskData, startOffset, ChangeMaskBits);
            if ((changeMask & (1 << 0)) != 0)
                snapshot.Linear_x = reader.ReadPackedFloatDelta(baseline.Linear_x, compressionModel);
            else
                snapshot.Linear_x = baseline.Linear_x;
            if ((changeMask & (1 << 0)) != 0)
                snapshot.Linear_y = reader.ReadPackedFloatDelta(baseline.Linear_y, compressionModel);
            else
                snapshot.Linear_y = baseline.Linear_y;
            if ((changeMask & (1 << 0)) != 0)
                snapshot.Linear_z = reader.ReadPackedFloatDelta(baseline.Linear_z, compressionModel);
            else
