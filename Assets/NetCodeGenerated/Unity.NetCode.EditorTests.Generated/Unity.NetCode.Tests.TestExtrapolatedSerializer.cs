...
{
    [BurstCompile]
    public struct UnityNetCodeTestsTestExtrapolatedGhostComponentSerializer
    {
        static GhostComponentSerializer.State GetState()
        {
            ...
            if (!s_StateInitialized)
            {
                ...
                {
                    GhostFieldsHash = 4677880248868524668,
                    ExcludeFromComponentCollectionHash = 1,
                    ComponentType = ComponentType.ReadWrite<Unity.NetCode.Tests.TestExtrapolated>(),
                    ComponentSize = UnsafeUtility.SizeOf<Unity.NetCode.Tests.TestExtrapolated>(),
        ...
        private static void CopyToSnapshot(IntPtr stateData, IntPtr snapshotData, int snapshotOffset, int snapshotStride, IntPtr componentData, int componentStride, int count)
        {
            for (int i = 0; i < count; ++i)
            {
                ...
                ref var component = ref GhostComponentSerializer.TypeCast<Unity.NetCode.Tests.TestExtrapolated>(componentData, componentStride*i);
        ...
        private static void CopyFromSnapshot(IntPtr stateData, IntPtr snapshotData, int snapshotOffset, int snapshotStride, IntPtr componentData, int componentStride, int count)
        {
            for (int i = 0; i < count; ++i)
            {
                ...
                var Value_DistSq = math.distancesq(snapshotBefore.Value, snapshotAfter.Value);
                if (Value_DistSq > 5625)
                    snapshotInterpolationFactor = 0;
                component.Value = math.lerp(snapshotBefore.Value,
                    snapshotAfter.Value, snapshotInterpolationFactor);
        ...
        {
            ref var component = ref GhostComponentSerializer.TypeCast<Unity.NetCode.Tests.TestExtrapolated>(componentData, 0);
            ref var backup = ref GhostComponentSerializer.TypeCast<Unity.NetCode.Tests.TestExtrapolated>(backupData, 0);
